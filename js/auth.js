...
async function login(event) {
    ...
    try {
        ...
        if (data.user && data.user.name) {
            console.log('FULL USER DATA RECEIVED:', JSON.stringify(data.user, null, 2));
            
            localStorage.setItem('userName', data.user.name);
            
            // Explicitly set user role with extra logging
            const userRole = data.user.role || 'user';
            console.log('Setting user role:', userRole, 'Received role:', data.user.role);
            localStorage.setItem('userRole', userRole);
            
            // Show success message
            await Swal.fire({
                title: 'Welcome Back!',
                text: `Hello ${data.user.name}, we're glad to see you again!`,
                icon: 'success',
            ...
            // Redirect logic
            if (userRole === 'admin') {
                console.error('ADMIN DETECTED - REDIRECTING TO ADMIN DASHBOARD');
